...
//For omap values ceph_test
int set_omap(rados_t cluster, rados_ioctx_t io, const char *oid) {
    int failed = 0;
    int ret,x1,x2,x3,buff_size = 50;
    char buffer1[100],buffer2[100],buffer3[100];
    int v1 = 53423,v2 = 78788 , v3 = 89378;
    x1 = snprintf(buffer1, buff_size, "   : Integer value ",v1);
    x2 = snprintf(buffer2, buff_size, "   : Integer value ",v2);
    x3 = snprintf(buffer3, buff_size, "   : Integer value ",v3);
    const char *keys[] = {"key1", "key2", "key3"};
    const char *vals[] = {buffer1, buffer2, buffer3};
    size_t lens[] = {x1, x2, x3};
    ...
    ret = rados_write_op_operate(wr, io, oid, NULL, 0);
    if (ret != 0) {
...
int print_one_omap(rados_ioctx_t io, const char *oid) {
    int failed = 0;
    int rval;
    int ret;
    ...
    ret = rados_read_op_operate(rd, io, oid, 0);
    if (ret != 0) {
...
*/
    failed = set_omap(cluster, io, "alpha");
    if (failed) {
        goto done;
    }
    failed = set_omap(cluster, io, "beta");
    if (failed) {
        goto done;
    }

    failed = print_one_omap(io, "alpha");
    if (failed) {
        goto done;
    }
    failed = print_one_omap(io, "beta");
    if (failed) {
        goto done;
    }
done:
